...
def get_mortality_rate(current_month):
...
def start_simulation_year():
    current_month = 1
    total_months = 0
    total_adults = tick_settings['adult_population']
    total_nymphs = tick_settings['nymph_population']
    total_larvae = tick_settings['initial_ticks'] - total_adults - total_nymphs

    print("STARTING SIMULATION")
    print(f"Initial ticks: {total_adults + total_nymphs + total_larvae}")
    print(f"Total adults: {total_adults}, Total nymphs: {total_nymphs}, Total larvae: {total_larvae}")

    while total_months < 12:  # Simulate for 12 months
        mortality_rate = get_mortality_rate(current_month)

        # Apply monthly mortality to adults and nymphs
        total_adults -= int(total_adults * mortality_rate)
        total_nymphs -= int(total_nymphs * mortality_rate)

        # Simulate egg laying and hatching
        eggs_laid = total_adults * tick_settings['eggs_laid_per_female']
        total_larvae += eggs_laid
        total_larvae -= int(total_larvae * mortality_rate)  # Apply mortality to larvae
        ...
        print(f"Total adults: {total_adults}, Total nymphs: {total_nymphs}, Total larvae: {total_larvae}")
        print(f"Total ticks: {total_adults + total_nymphs + total_larvae}")
